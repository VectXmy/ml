...
# -*- coding:utf-8 -*-
'''
The code that was written according to Andrew Ng's course.
commented by xxxmy.
...
class NeuralNetwork:
    def __init__(self, layers, activation='tanh'): #layers :层数，包含输入层。为list
        ...
        self.weights = []
        for i in range(len(layers)-1):#这里len(layers)=3
            # [0,1) * 2 - 1 => [-1,1) => * 0.25 => [-0.25,0.25)  #这里1次对两层w赋值
        ...
        for k in range(epochs):
            ...
            for j in range(layerNum, 0, -1): # 倒数第二层开始,-1表示反序，所以求出的delta顺序应该反的
               # deltas.append(deltas[-1].dot(self.weights[j].T) * self.activation_deriv(a[j]))
                 deltas.append(self.weights[j].T.dot(deltas[-(layerNum+1-j)]) * self.activation_deriv(a[j]))
            ...
            for i in range(len(self.weights)):
                ...
                self.weights[i] += learning_rate * (delta.dot(al.T)) 
